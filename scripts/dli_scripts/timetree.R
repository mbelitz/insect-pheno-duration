...
itree_otl = ape::read.tree("data/tree_otl.tre")
itree_otl = timetree_phylo(itree_otl)
...
phylocomr::ph_bladj(ages = "data/age_test", phylo = "data/phy_test.tre")
...
phy = read.tree("data/phy_test.tre")
phy2 = timetree_phylo(phy)
...
phy3$edge.length
phy4 = ape::chronos(phy3, calibration = cali, model = "relaxed")
plot(phy4); tiplabels(); nodelabels()
phy4$edge.length

select(phy2$info, node, estimated) %>% 
  write.table(file = "data/age_test", quote = F, row.names = F, col.names = F)
write.tree(phy2$phy, file = "data/phy_test.tre")
age = read.table("data/age_test")
write.table(age, "data/age_test", sep = "\t", row.names = F, col.names = F, quote = F)
