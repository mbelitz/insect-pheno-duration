...
dur <- read.csv("phenesse_outputs/total_25km/total_duration_25km.csv", stringsAsFactors = FALSE)
...
total_dur_clim_pop <- total_dur_clim_pop %>% 
  dplyr::select(onset, onset_low, onset_high, year, offset, offset_low, offset_high, 
                duration, scientificName, Order, lon, lat, OS, prec, temp, bio4, bio15, pop)

write.csv(total_dur_clim_pop, file = "data/model_dfs/duration_climate_population_data_25km.csv", row.names = FALSE)
